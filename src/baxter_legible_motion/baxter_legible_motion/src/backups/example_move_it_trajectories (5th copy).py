...
def main():
    ...
    # total_plan = []
    # total_plan.append( iter_points[0] )
    ...
    obstaclePositions = [ np.array([0.77, 0.22, -0.03]), np.array([0.77, 0.02, -0.03]), np.array([0.77, -0.18, -0.03]), np.array([0.77, -0.38, -0.03]) ]
    xy_array = np.array([-1, -1, 0])
    z_array = np.asarray([ 0, 0, 1 ])
    targetPosition = np.array([0.77, -0.58, -0.03])
    # epsilon = 0.005
    # k_rep = 0.1
    # k = 0.1
    # epsilon = 0.005
    ...
    while True:
        ...
        while True:
            ...
            # for i in range(len(iter_points) - 1):
            # # for i in range(len(iter_points) - 2):
            #     iter_points[i + 1][0] = ( ( - (total_plan[i][0] - total_plan[i + 1][0]) / np.linalg.norm(total_plan[i][0] - total_plan[i + 1][0]) ) * ro_max ) + total_plan[i][0]

            # # stop rule and list changing
            # to_remove = []
            # for i in range(1, len(iter_points) - 1):
            #
            #     print(f'np.linalg.norm(iter_points[i][0] - iter_points_prev[i][0]): {np.linalg.norm(iter_points[i][0] - iter_points_prev[i][0])}')
            #
            #     if np.linalg.norm(iter_points[i][0] - iter_points_prev[i][0]) <= epsilon:
            #
            #         # if to_remove
            #         total_plan.append( iter_points[i] )
            #         # if total_plan[i][1] == iter_points[i][1]:
            #         #     total_plan.append( iter_points[i] )
            #         # else:
            ...
            # stop rule and list changing
            to_remove = []
            # for i in range(1, len(total_plan) - 1):
            # for i in range(len(total_plan) - 1):
            for i in range(len(total_plan)):
                ...
                if np.linalg.norm(total_plan[i][0] - total_plan_prev[i][0]) <= epsilon:
                    to_remove.append(i)

            print(f'to_remove: {to_remove}')

            # # check if to_remove is increasing by 1 and starts with 0
            # arr = np.array(to_remove)
            # diff = np.diff(arr)
            # is_increasing = np.all(diff == 0)
            # if is_increasing and arr == 0:
            #     print('all good')
            # else:
            #     print('fail')
        ...
        # print(f'total_plan before: {total_plan}')
        #
        # for i in range(len(total_plan)):
        #      total_plan[i][1] = i
        #
        ...
        # stop_rule_add_iter_points = np.linalg.norm(total_plan[-1][0] - targetPosition)
        # print(f'stop_rule_add_iter_points: {stop_rule_add_iter_points}')
        # dir_vector = ( - (total_plan[-1][0] - targetPosition) / np.linalg.norm(total_plan[-1][0] - targetPosition) )
    ...
    # for i in range(len(iter_points)):
    #     wpose.position.x = iter_points[i][0][0]
    #     wpose.position.y = iter_points[i][0][1]
    #     wpose.position.z = iter_points[i][0][2]
