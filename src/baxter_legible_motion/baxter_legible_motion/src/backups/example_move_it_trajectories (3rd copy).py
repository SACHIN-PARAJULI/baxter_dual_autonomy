...
def main():
    ...
    # p = PoseStamped()
    # p.header.frame_id = example.robot.get_planning_frame()
    # p.pose.position.x = 0.7
    # p.pose.position.y = 0.0
    # p.pose.position.z = -0.5
    # example.scene.add_box("table", p, (0.5, 1.5, 0.7))
    ...
    # if not np.linalg.norm( Q_update - Q_prev) <= epsilon:
        ...
        while index < length_plan2-1:
            ...
            derivative_ro_b_1 = np.asarray([ (eePosition[0] - obstaclePosition_1[0])/ro_b_1 , (eePosition[1] - obstaclePosition_1[1])/ro_b_1 , 0 ])
            U_potential_gradient_rep_1 = (1/ro_b_1) * (1/(ro_b_1**2)) * derivative_ro_b_1
            ...
            derivative_ro_b_2 = np.asarray([ (eePosition[0] - obstaclePosition_2[0])/ro_b_2 , (eePosition[1] - obstaclePosition_2[1])/ro_b_2 , 0 ])
            U_potential_gradient_rep_2 = (1/ro_b_2) * (1/(ro_b_2**2)) * derivative_ro_b_2
            ...
            derivative_ro_b_3 = np.asarray([ (eePosition[0] - obstaclePosition_3[0])/ro_b_3 , (eePosition[1] - obstaclePosition_3[1])/ro_b_3 , 0 ])
            U_potential_gradient_rep_3 = (1/ro_b_3) * (1/(ro_b_3**2)) * derivative_ro_b_3
            ...
            derivative_ro_b_4 = np.asarray([ (eePosition[0] - obstaclePosition_4[0])/ro_b_4 , (eePosition[1] - obstaclePosition_4[1])/ro_b_4 , 0 ])
            U_potential_gradient_rep_4 = (1/ro_b_4) * (1/(ro_b_4**2)) * derivative_ro_b_4
            ...
            # position:
            #   x: 0.5781738419680907
            #   y: -0.1840141310125901
            #   z: 0.11206425212603752

            # obstaclePosition_5 = (0.58, -0.18, 0.78)
            # ro_b_5 = math.dist(eePosition, obstaclePosition_5)
            # derivative_ro_b_5 = np.asarray([ (eePosition[0] - obstaclePosition_5[0])/ro_b_5 , (eePosition[1] - obstaclePosition_5[1])/ro_b_5 , (eePosition[2] - obstaclePosition_5[2])/ro_b_5 ])
            # U_potential_gradient_rep_5 = (1/ro_b_5) * (1/(ro_b_5**2)) * derivative_ro_b_5
            # # U_potential_gradient_rep_5 = k_rep * (1/ro_b_5) * (1/(ro_b_5**2)) * derivative_ro_b_5
            ...
            # dir_5 = z_array*(eePosition - obstaclePosition_5) / np.linalg.norm(z_array*(eePosition - obstaclePosition_5))
            # # dir_5 = z_array
            # print('dir_5')
            # print(dir_5)
            # # with respect to ee and target direction
            # # dir_4 = dir_4 - dir_target
            # U_potential_gradient_rep_5_sphere = U_potential_gradient_rep_5 * dir_5
            # print('U_potential_gradient_rep_5_sphere')
            # print(U_potential_gradient_rep_5_sphere)
            ...
            print('psi_4')
            print(psi_4)
            # psi_5 = np.linalg.norm( np.cross( (obstaclePosition_5-eePosition) , (targetPosition-eePosition) ) ) / np.linalg.norm( (targetPosition-eePosition) )
            # print('psi_5')
            # print(psi_5)
            ...
            # U_potential_gradient = xi * ( (U_potential_gradient_rep_1_cylinder+U_potential_gradient_rep_1_sphere)*psi_1 + (U_potential_gradient_rep_2_cylinder+U_potential_gradient_rep_2_sphere)*psi_2 + (U_potential_gradient_rep_3_cylinder+U_potential_gradient_rep_3_sphere)*psi_3 + (U_potential_gradient_rep_4_cylinder+U_potential_gradient_rep_4_sphere)*psi_4 + U_potential_gradient_rep_5_sphere*200 )
